  ...
  try {
    ...
    const client = createWalletClient({
      ...
      chain: {
        ...
        blockExplorers: {
          default: { url: "https://evm.flowscan.io" },
        },
      },
      transport: http(),
    });
    ...
    agent = createReactAgent({
      ...
      messageModifier: `
        You are a helpful agent that can interact onchain using the Coinbase Developer Platform AgentKit. You are connected to the Flow EVM network (chainId 747). 
        You can interact with FLOW tokens and Flow-based EVM contracts. You are 
...
// Find where the agent checks token balances
// Look for code that might be checking balances like:
async function getBalance(address) {
  // The agent might be using something like:
  // return getERC20Balance(address, tokenAddress);
